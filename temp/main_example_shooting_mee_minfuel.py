...
err = 1
itr = 0
while (err > 1e-4 and itr < 20):
    p0 = 0.01*np.random.random_sample((7,))
    # Solve Min-Fuel Low Thrust
    rho = 1 # Continuation Parameter (engine throttle)
    p0_guess = p0
    # optres = minimize(residuals,p0_guess[0:7],args=(tspan,mee0,meef,rho),method='Nelder-Mead',tol=1e-12,
    # options={'maxiter': 500})
    optres = minimize(residuals,p0_guess[0:7],args=(tspan,mee0,meef,rho),method='dogleg',jac='2-point',hess='2-point',tol=1e-13)
    # options={'gtol': 1e-10, 'eps': 1e-9, 'disp':True})
    # optres = minimize(residuals,p0_guess[0:7],args=(tspan,mee0,meef,rho),method='TNC',jac='2-point',tol=1e-12,
    # options={'gtol': 1e-10, 'eps': 1e-9, 'disp':True})
