## @package eclipse_model
#  \brief     Eclipse model
#  \details   Grazing goat eclipse model and cylindrical eclipse model
#  \author    Robyn Woollands
#  \pre       numpy, const.py
#  \bug       No bugs known
#  \warning   A better smoothing technique is required to allow full use of the
# grazing goat model
...
def eclipse_grazing_goat(t,X,rho):
    """
    Smoothed grazing goat eclipse model
    Parameters:
    ===========
    t   -- time
    X   -- state vector
    ...
    if rho < 1:
        if (dot(-(rSC-Xp),rS) >= norm(rS-Xp)*cos(alpha)):
            ...
            if (d < 1e-10):
                ...
                # NOTE: Once a better smoothing technique has been determined,
                # the following lines should be uncomented to allow for use of
                # grazing goat model
...
def eclipse_cylindrical(t,X,rho):
    """
    Smoothed cylindrical eclipse model
    Parameters:
    ===========
    t   -- time
    X   -- state vector
